...
pub(crate) async fn admin_force_delete_machine(
    ...
    let instance_id = if let Some(host_machine) = &host_machine {
        // Instance allocation holds the host Machine row while it inserts the
        // Instance. The state update below acquires that same row lock. Read
        // membership only after it returns so an allocation that was already
        // in progress is included in this request. Under READ COMMITTED, the
        // re-read sees the row committed by the writer that held the lock.
        db::machine::advance(
            host_machine,
            &mut txn,
