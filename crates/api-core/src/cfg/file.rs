...
pub struct CarbideConfig {
    ...
    /// pool closes it (sqlx's own default, pinned explicitly). At its 10m
    /// default this keeps the pool's reaping well inside the Postgres
    /// server's sixty-minute idle-session reaper. Default is 10m.
    #[serde(
