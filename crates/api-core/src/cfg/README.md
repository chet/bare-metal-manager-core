...
| `database_pool_idle_timeout` | `Duration` | `10m` | `server` | Idle time after which the pool closes a connection; at its default this stays well inside the Postgres server's 60-minute idle-session reaper. Must be greater than zero (startup rejects `0`). |
