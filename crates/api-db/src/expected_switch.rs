...
pub async fn create(
    ...
    sqlx::query_as(query)
        ...
        .bind(&switch.data.bmc_username)
        .bind(&switch.data.bmc_password)
        .bind(&switch.data.serial_number)
        .bind(&switch.data.metadata.name)
        .bind(&switch.data.metadata.description)
        .bind(&switch.data.rack_id)
        .bind(sqlx::types::Json(&switch.data.metadata.labels))
        .bind(&switch.data.nvos_username)
        .bind(&switch.data.nvos_password)
        .bind(&switch.data.nvos_mac_addresses)
        .fetch_one(txn)
        .await
