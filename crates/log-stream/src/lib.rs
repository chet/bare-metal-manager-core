...
//! In-process `tracing` tap for the admin web UI live log viewer.
//!
//! This crate is shared by `carbide-api` and the scout agent so both install the
//! same tap and stream their logs to the viewer with identical structure.
//!
//! [`LogStreamLayer`] is installed in a `tracing` subscriber registry and
//! forwards to a [`LogStream`] — a bounded broadcast channel (for live tailing)
//! plus a small byte-capped ring buffer (for replay-on-connect and scrollback):
...
/// Default in-memory byte budget for retained history, used by
/// [`LogStream::default`]. Callers override it via [`LogStream::with_max_bytes`]
/// (e.g. from a configured value). 128 MiB.
...
impl LogLine {
    /// Normalize an arbitrary level string back to one of the static level
    /// labels this tap emits. Used when reconstructing a line received over the
    /// wire (e.g. scout log lines proxied through carbide-api) so the viewer sees
    /// the same set of levels as for in-process lines. Unknown values fall back
    /// to `"INFO"`.
...
impl LogStream {
    ...
    /// budget (e.g. from a configured `max_megabytes`).
